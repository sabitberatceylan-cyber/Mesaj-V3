...
export default function SohbetEkrani({ sunucuAdres, token, onCikis, onAdresGuncelle }) {
  ...
  const baglan = useCallback(() => {
    ...
    ws.onmessage = (olay) => {
      ...
      if (veri.tip === 'hosgeldin') {
        ...
        setDigerKullanici(veri.digeri || null);
        return;
      }
  ...
  function gonder() {
    ...
    if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) {
      Alert.alert(
        'Bağlı değil',
        'Sunucuya bağlanamadım. Sağ üstteki ⚙ simgesinden sunucu adresinin güncel olduğunu kontrol et.'
      );
      return;
    }
    ...
    <View style={styles.kok}>
      ...
      <Modal visible={ayarAcik} animationType="slide" transparent>
        <View style={styles.modalArkaplan}>
          <View style={[styles.modalKutu, { paddingBottom: Math.max(insets.bottom, bosluk.lg) + 20 }]}>
...
const styles = StyleSheet.create({
  ...
  headerBaslik: { color: renkler.metin, fontSize: 16, fontWeight: '700' },
  headerAltyazi: { color: renkler.metinSoluk, fontSize: 11, marginTop: 1 },
  ayarIkon: { color: renkler.metin, fontSize: 22 },
